  ...
  async _processLine(student, rest, config) {
    ...
    for (let raw of parts) {
      ...
      if (vm && menus[vm[1].trim()]) {
        vendor = vm[1].trim(); key = vm[2].trim();
      }
      if (!menus[vendor]) { missing.push(raw); continue; }
      let qty = 1;
      ...
      if (price == null) {
        const keys = Array.isArray(Object.keys(menus[vendor])) ? Object.keys(menus[vendor]) : [];
        if (typeof itemName === 'string' && keys.length) {
